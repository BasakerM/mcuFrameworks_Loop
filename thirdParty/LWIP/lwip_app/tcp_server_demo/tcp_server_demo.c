...
unsigned char tcp_server_sendbuf[256] = {0};
...
void tcp_server_test(void)
{
	...
	while(res==0)
	{
		...
		if(tcp_server_flag&1<<6) //是否收到数据
		{
			...
			sprintf((char*)tcp_server_sendbuf,(char*)tcp_server_recvbuf);
...
void tcpServerDemoDelayMs(unsigned short ms);
...
void tcp_server_remove_timewait(void)
{
	...
	while(tcp_active_pcbs!=NULL)
	{
		...
		// mTime.delayMs(10);//等待tcp_active_pcbs为空  
		tcpServerDemoDelayMs(10);
	}
	pcb=tcp_tw_pcbs;
